...
public class PlanarReflectionsProbe : MonoBehaviour {
    ...
    [Space(10)]
    [Range(0.1f, 1)] public float reflectionsResolution = 1f;
    public float targetDistance = 250f;
    ...
    private void OnEnable () {
        Camera.onPreCull += PreRenderRoutine;
    }
    ...
    private void CreateRenderTexture(Camera cam) {
        ...
        float distanceAmount = 0.2f;

        if (distance < targetDistance)
        {
            distanceAmount = Mathf.Clamp((targetDistance - distance) / distance, 0.2f, 1f);
        }
        
        int width = (int)(cam.pixelWidth * reflectionsResolution * distanceAmount);
        int height = (int)(cam.pixelHeight * reflectionsResolution * distanceAmount);
    ...
    public static PlanarReflectionsProbe[] FindProbesRenderingTo (int id) {
        var probes = FindObjectsByType<PlanarReflectionsProbe>(FindObjectsSortMode.None);
