    ...
    public class Horizonter : MonoBehaviour
    {
        ...
        protected virtual void UpdateElements()
        {
            ...
            foreach (Transform child in spawnParent.GetComponentsInChildren<Transform>())
            {
                ...
                {
                    if (shownPresets.Contains(element.GetPreset()))
                        continue;
                    else
            ...
            if (disableIfNotShown && shownPresetsCount <= notShownCount && Application.isPlaying)
            {
            ...
            if (elements.Count > shownPresetsCount)
            {
                var last = elements.TakeLast(elements.Count - shownPresetsCount);
            ...
            for (int i = 0; i < shownPresetsCount; i++)
            {
                var preset = shownPresets[i];
                var realIndex = presets.IndexOf(preset);

                if (elements.Count < shownPresetsCount || elements[i] == null)
                {
                ...
                {
                    elements[i].SetPreset(realIndex);
                }
            }
        }
